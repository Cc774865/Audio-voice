...
`lexicon_match` 先把正确稿用 jieba 分词，只有落在**真实词边界**上的组词才算命中（否则「题目中的」会把片段「中的」误配成 zhòng dì）。非成词片段不再作为语境依据，回退到「的地得」规则和 lazy_pinyin。

## 复审读音受控

复审参考 TTS 会让语境确定的字读准：把该字换成**同音同调、默认读音唯一**的常见字再合成（「行 háng」→「杭」，「率 lǜ」→「律」，「得 dé」→「德」），旁路记录，比对前还原。细节见 [review.md](review.md)。
