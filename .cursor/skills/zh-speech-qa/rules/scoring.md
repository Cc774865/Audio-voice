...
不要重训 FunASR。用记忆库里的典型例标注跑 `.cursor/skills/zh-speech-qa/scripts/calibrate.py`：**每满 60 条**才出 `LOW_SCORE` / CER / 停顿拖音毫秒 / `BASE` `COMPRESS` 建议。脚本只打印，不改本文件。你确认后才把数字写进 `score.py` 和这里。问完后跑 `calibrate.py --ack`。
...
检查 + 复审合成的「错误 / 不合格 / 待审 / 合格」见 [review.md](review.md)，**不改**上面的综合分公式。
